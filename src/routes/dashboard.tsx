...
import {
  Video, MapPin, Phone, ArrowUpRight, AlertTriangle, CalendarClock,
  MessageCircle, TrendingUp, FileText, CheckCircle2, XCircle,
} from "lucide-react";
import {
...
function Dashboard() {
  ...
  const newIntakes = INTAKES.length;
  return (
    <div className="max-w-[1400px] mx-auto px-5 lg:px-8 py-6 lg:py-8">
      {/* Briefing */}
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-[clamp(1.6rem,2.4vw,2rem)] tracking-tight leading-tight" style={{ fontFamily: "'Fraunces', serif", color: ink }}>
            {greeting()}, Dr. Sharma
          </h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Stat label="new intakes" value={String(newIntakes)} />
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <section className="col-span-12 lg:col-span-8 rounded-2xl p-5" style={cardStyle}>
          <Header title="Today's schedule" hint={`${SESSIONS_TODAY.length} sessions`} to="/schedule" />
            ...
            {SESSIONS_TODAY.map((s) => {
              ...
              return (
                <div key={s.id} className="py-3 flex items-center gap-3">
                  ...
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] text-white shrink-0" style={{ background: primary }}>
                    {p?.initials}
                  </div>
        ...
        <section className="col-span-12 lg:col-span-4 rounded-2xl p-5" style={cardStyle}>
          <Header title="Alerts" hint={`${ALERTS.length} active`} to="/patients" />
          <div className="mt-4 space-y-2.5">
            {ALERTS.map((a) => {
              const p = getPatient(a.patientId);
              const tone = a.priority === "high" ? { bg: "#FDECEC", ink: "#B54848", border: "#F3C7C7" }
                : a.priority === "medium" ? { bg: soft, ink: primary, border: border }
                : { bg: surface2, ink: muted, border: border };
              return (
                <div key={a.id} className="rounded-xl p-3" style={{ background: tone.bg, border: `1px solid ${tone.border}` }}>
        ...
        <section className="col-span-12 lg:col-span-5 rounded-2xl p-5" style={cardStyle}>
          <Header title="Needs follow-up" hint="Overdue since last session" to="/patients" />
          <div className="mt-3 divide-y" style={{ borderColor: border }}>
            {PATIENTS.filter((p) => p.lastSession).slice(0, 5).map((p) => (
              <div key={p.id} className="py-2.5 flex items-center gap-3">
                ...
                <button className="text-[11px] px-2.5 py-1 rounded-full" style={{ background: surface2, color: ink, border: `1px solid ${border}` }}>
        ...
        {/* Revenue */}
        <section className="col-span-12 lg:col-span-4 rounded-2xl p-5" style={cardStyle}>
          <Header title="Revenue · this month" hint="INR" to="/billing" />
          <div className="mt-4 flex items-end gap-4">
            <div>
              <div className="text-[26px] leading-none tabular-nums" style={{ fontFamily: "'Fraunces', serif", color: ink }}>
                ₹{(REVENUE_MONTH.completed / 1000).toFixed(0)}k
              </div>
              <div className="text-[11px] mt-1 flex items-center gap-1" style={{ color: "#1F7A3E" }}>
        ...
        <section className="col-span-12 lg:col-span-3 rounded-2xl p-5" style={cardStyle}>
          <Header title="Recent notes" to="/notes" />
        ...
        <section className="col-span-12 lg:col-span-6 rounded-2xl p-5" style={cardStyle}>
          <Header title="Weekly load" hint="Booked vs open slots" to="/schedule" />
        ...
        {/* Intake requests */}
        <section className="col-span-12 lg:col-span-6 rounded-2xl p-5" style={cardStyle}>
          <Header title="New intake requests" hint={`${INTAKES.length} pending`} to="/patients" />
...
// Wrap default export layout via Route.component only — but we still need shell
export const Component = Dashboard;

function DashboardWithShell() {
  return <AppShell crumb="Dashboard"><Dashboard /></AppShell>;
}
// Override component to use shell
Route.options.component = DashboardWithShell;

function Header({ title, hint, to }: { title: string; hint?: string; to?: string }) {
  return (
...
void FileText; void CalendarClock;
